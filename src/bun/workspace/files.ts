...
import { PiWorkspaceFileRequestSchema, type PiWorkspaceFile, type PiWorkspaceFileContent, type PiWorkspaceFileRequest } from "@shared/pi-contract";
...
export async function listWorkspaceFiles(input: PiWorkspaceFileRequest): Promise<PiWorkspaceFile[]> {
	const request = PiWorkspaceFileRequestSchema.parse(input);
	const directory = await resolveExistingWorkspacePath(request.workspacePath, request.relativePath);
	const entries = await readdir(directory, { withFileTypes: true });
	return entries
		.filter((entry) => !HIDDEN_DIRECTORIES.has(entry.name))
		.map((entry) => ({
			name: entry.name,
			path: toRelativePath(request.relativePath, entry.name),
...
export async function readWorkspaceFile(input: PiWorkspaceFileRequest): Promise<PiWorkspaceFileContent> {
	const request = PiWorkspaceFileRequestSchema.parse(input);
	if (!request.relativePath) throw new Error("请选择要预览的文件。");
	const filePath = await resolveExistingWorkspacePath(request.workspacePath, request.relativePath);
	const fileStat = await stat(filePath);
	if (!fileStat.isFile()) throw new Error("所选路径不是文件。");
	const content = await readFilePrefix(filePath, fileStat.size);
	const isBinary = content.includes(0);
	const isTruncated = fileStat.size > MAX_FILE_BYTES;
	return {
